...
const run = async () => {
  ...
  // We test a candidate set of offsets. Let's see if we can find one with low score, or just print violations for a set of offsets
  // Let's use offsets that are randomly chosen but let's print violations for a candidate with score 31.
  // We can run a small local search here and print violations for the best result.
  const employeeIdToIndexMap = new Map();
  employees14.forEach((emp, i) => {
    employeeIdToIndexMap.set(emp.employeeId, i);
  });

  const partitions = actualPartitions.map(part => {
    ...
    const datesInfo = part.dates.map(date => {
      ...
      return { date, absoluteDayIndex, isWeekend };
    });
    ...
    for (let idx = 0; idx < partitions.length; idx++) {
      ...
      for (let d = 0; d < part.datesInfo.length; d++) {
        ...
        for (let i = 0; i < 14; i++) {
          ...
          if (shift === 0) {
            morningCount++;
            morningMembers.push(emp);
            if (emp.isSenior) morningSeniors++;
          } else if (shift === 1) {
            eveningCount++;
            eveningMembers.push(emp);
            if (emp.isSenior) eveningSeniors++;
          } else {
            nightCount++;
            nightMembers.push(emp);
        ...
        if (morningCount < requiredMin) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Morning has only ${morningCount} working (needed ${requiredMin}). Working: ${morningMembers.map(m => m.employeeId).join(",")}`);
        }
        if (eveningCount < requiredMin) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Evening has only ${eveningCount} working (needed ${requiredMin}). Working: ${eveningMembers.map(m => m.employeeId).join(",")}`);
        }
        if (nightCount < requiredMin) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Night has only ${nightCount} working (needed ${requiredMin}). Working: ${nightMembers.map(m => m.employeeId).join(",")}`);
        }

        if (morningSeniors < 1) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Morning has 0 seniors.`);
        }
        if (eveningSeniors < 1) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Evening has 0 seniors.`);
        }
        if (nightSeniors < 1) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Night has 0 seniors.`);
        }

        if (nightFemales === 1) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Night has exactly 1 female working: ${nightMembers.filter(m => m.isFemale).map(m => m.employeeId).join(",")}`);
        }
      }
    }
