...
const createShiftTeams14 = (employees, monthStart, prevEveningTeamIds = null) => {
  ...
  // Evening Team: remaining 6 members (1 female senior, 2 male seniors, 3 juniors)
  ...
  const evaluate = (offsets, print = false) => {
    ...
    for (let idx = 0; idx < partitions.length; idx++) {
      ...
      for (let d = 0; d < part.datesInfo.length; d++) {
        ...
        if (nightCount < nightRequiredMin) {
          failures++;
          if (print) console.log(`Violation [${dateInfo.date}] Night has only ${nightCount} working (needed ${nightRequiredMin}). Working: ${nightMembers.map(m => m.employeeId).join(",")}`);
        }
