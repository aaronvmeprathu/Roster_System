...
const runSearch = async () => {
  ...
  const evaluate = (offsets) => {
    ...
    for (let idx = 0; idx < actualPartitions.length; idx++) {
      ...
      for (const date of part.dates) {
        ...
        const requiredMin = shiftKey === "night" ? SHIFT_MINIMUMS.night : (isWeekend ? 2 : 3);
        ...
        for (const shiftKey of SHIFT_KEYS) {
          ...
          if (assigned.length < requiredMin) failures++;
  ...
  if (bestScore === 0) {
    console.log("FOUND 14-EMPLOYEE CONFIGURATION!");
    console.log(JSON.stringify(bestOffsets, null, 2));
  } else {
    console.log("Failed to find. Best score:", bestScore);
  }
};
