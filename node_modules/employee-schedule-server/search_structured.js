...
const runSearch = async () => {
  ...
  console.log("Starting structured search...");
  const t0 = Date.now();
  const maleAllowedOffsets = [0, 1, 5, 6];

  for (let restart = 0; restart < 50000; restart++) {
    ...
    while (improved && steps < 100) {
      ...
      for (let i = 0; i < 14; i++) {
        ...
        if (emp.gender === "female") {
          // Females can try other offsets, maybe weekend offset doesn't have to be exactly 3?
          // Let's allow females to try 3 or other weekend offsets (none really, Saturday/Sunday off is exactly 3).
          // But let's allow them to try all 7 just in case.
