...
const employees14 = employeeSeed.map(emp => {
...
const createShiftTeams14 = (employees, monthStart, prevEveningTeamIds = null) => {
  ...
  // Night Team: 2 female seniors, 1 male senior, 1 male junior (Total 4)
  ...
  // Evening Team: remaining 6 members (0 female seniors, 2 male seniors, 4 juniors)
...
const runSearch = async () => {
  ...
  const evaluate = (offsets) => {
    ...
    for (let idx = 0; idx < partitions.length; idx++) {
      ...
      for (let d = 0; d < part.datesInfo.length; d++) {
        ...
        if (nightCount < nightRequiredMin) failures++;
  ...
  console.log("Searching with frozen females...");
  const t0 = Date.now();
  ...
  for (let restart = 0; restart < 50000; restart++) {
    ...
    while (improved && steps < 100) {
      ...
      for (let i = 0; i < 14; i++) {
        const emp = employees14[i];
        if (emp.gender === "female") continue; // keep frozen!

        const originalVal = current[i];
        let bestNewVal = originalVal;
        let bestNewScore = score;
        
        // Males can try any of the weekday allowed offsets
        for (const newVal of maleAllowedOffsets) {
  ...
  console.log("Best offsets:", JSON.stringify(bestOffsets));
  const offsetMap = {};
  employees14.forEach((emp, i) => {
